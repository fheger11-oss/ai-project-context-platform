...
| Feedback            | Feedback id, user id, feedback type, message, optional SPA pathname, created timestamp                                                                                                 | PostgreSQL `feedback`                                 | Founder product feedback review                            | User-authored product feedback; may contain personal or sensitive details if the user includes them              | Indefinite until manual deletion or user deletion                           | User deletion cascades feedback                                                          |
...
Authenticated users can submit feedback through `POST /api/v1/feedback`. Feedback is stored in PostgreSQL `feedback` with one of four types: `FEATURE_REQUEST`, `BUG`, `CONFUSING`, or `GENERAL`. The authenticated session supplies `userId`; clients cannot submit feedback for another user.

After a successful save, the frontend sends a lightweight PostHog `feedback_submitted` event with only `type` and `page`. The feedback message, user email, repository names, repository contents, tokens, API response bodies, source code, and other sensitive payloads are not sent to PostHog. The current founder workflow is to review rows directly in the Supabase Table Editor.
