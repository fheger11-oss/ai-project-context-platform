...
describe("Analysis domain boundaries", () => {
  ...
  it("does not expose credential-bearing fields in analysis contracts", () => {
    expect(source).not.toMatch(/authorization|credential|accessToken|refreshToken|githubToken/i);
  });

  it("does not duplicate scan lifecycle or repository content provider contracts", () => {
    expect(source).not.toMatch(/RepositoryContentProvider|resolveCommit|createScan|ScanFile/i);
  });
});
